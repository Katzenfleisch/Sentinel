...
local function GetOptimalMoveDirection( self, from, to )
   ...
   if self.currPathPoints == nil or self.forcePathRegen or from:GetDistanceTo(to) < minDistOpti then
      ...
      if self.currPathPoints and #self.currPathPoints > 0
      then
...
function BotMotion:GetMotionTick()
   return kAIA_BotMotion_tick -- 6 -- This value is good enough so far
end
    ...
    if moveTargetPos ~= nil and not player:isa("Embryo") then
        ...
        if distToTarget <= 0.01 then
            ...
            if updateMoveDir then
               ...
               self.nextMoveUpdate = now + 1 / self:GetMotionTick()
               ...
               then
                ...
                elseif distToTarget <= 2.0 then
                    ...
                    GetOptimalMoveDirection( self, currentPos, moveTargetPos )
...
function BotMotion:GetPathIndexForOrig(orig, fromIt, toIt)
   if self.currPathPoints then
      local closest = nil
      local from = fromIt and Clamp(fromIt, 1, #self.currPathPoints) or 1
      local to   = toIt   and Clamp(toIt,   1, #self.currPathPoints) or #self.currPathPoints

      for i = from, to do
         if not closest
            or self.currPathPoints[i]:GetDistanceTo(orig) < self.currPathPoints[closest]:GetDistanceTo(orig)
         then
            closest = i
         end
      end
